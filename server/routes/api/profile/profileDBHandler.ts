...
export default class DBHandler {
  async getBabySitterProfile(
    ...
    const babysitterQuery = `SELECT babysitter_name AS "name",
                                 ...
                                 AND end_timestamp = ($2)`;
    const profile = await db.query(babysitterQuery, [
      babysitterId,
      END_TIMESTAMP,
    ]);
    return profile.rows;
  ...
  async updateBabySitterProfile(
    babysitterId: number,
    babysitterData: BabysitterUpdate
  ): Promise<void> {
    const fields: string[] = [];
    const params = [];
    let paramIndex = 1;
